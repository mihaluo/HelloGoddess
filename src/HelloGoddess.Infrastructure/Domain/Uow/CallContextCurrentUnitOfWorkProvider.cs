...
namespace HelloGoddess.Infrastructure.Domain.Uow
{
    ...
    {
        private static readonly ThreadLocal<Dictionary<string, string>> ThreadLocal = new ThreadLocal<Dictionary<string, string>>(ValueFactory);

        private static Dictionary<string, string> ValueFactory()
        {
        ...
        internal static void FreeNamedDataSlot(string contextKey)
        {
            ...
            {
                if (ThreadLocal.Value.ContainsKey(contextKey))
                {
                    ThreadLocal.Value.Remove(contextKey);
                }
            }
        }
        ...
        internal static string LogicalGetData(string contextKey)
        {
            if (ThreadLocal.IsValueCreated)
            {
                return ThreadLocal.Value[contextKey];
            }
            return null;
        }
        ...
        {
            ThreadLocal.Value.Add(contextKey, outerUnitOfWorkKey);
        }
    }
}