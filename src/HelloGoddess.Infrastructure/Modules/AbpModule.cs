...
namespace HelloGoddess.Infrastructure.Modules
{
    ...
    public abstract class AbpModule
    {
        ...
        {
            var type = moduleType.GetTypeInfo();
            if (!IsAbpModule(moduleType))
            {
                throw new AbpInitializationException("This type is not an ABP module: " + type.AssemblyQualifiedName);
            }

            var list = new List<Type>();

            if (type.IsDefined(typeof(DependsOnAttribute), true))
            {
                var dependsOnAttributes = type.GetCustomAttributes(typeof(DependsOnAttribute), true).Cast<DependsOnAttribute>();
        ...
        public static List<Type> FindDependedModuleTypesRecursivelyIncludingGivenModule(Type moduleType)
        {
            ...
            //list.AddIfNotContains(typeof(AbpKernelModule));//todo yw
            return list;
        }
